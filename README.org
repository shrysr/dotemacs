...
(custom-set-faces
 ...
 '(org-level-3 ((t (:inherit outline-3 :height 0.9))))
 '(org-level-4 ((t (:inherit outline-4 :height 0.8))))
 '(org-level-5 ((t (:inherit outline-5 :height 0.750))))
 )
#+end_src
...
* Temp workaround for magit Issue
...
: 1
* Theme and visuals
** TEST Emacsclient or frame specific settings
...
** Background change to light Grey

This is suitable to use with the Leuven theme. However, it has to be disabled to enable other themes.
...
** Custom Safe themes and using Zenburn
...
** TEST Org babel source block background Color
...
** STABLE Spaceline : modeline configuration
CLOSED: [2019-03-27 Wed 20:43]
- Note taken on [2019-03-27 Wed 20:42] \\
  This is reasonably well customised and useful. Switching to stable state.
